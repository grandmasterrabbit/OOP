...
Complex::Complex() {
     ...
     */
    this->value = std::make_pair<double, double>(0, 0);
}
...
Complex::Complex(float _val1, float _val2) {
     ...
     */
    this->value = std::make_pair<double, double>(_val1, _val2);
}
...
Number *Complex::add(Number * num) {
    ...
    if (num->types() == INTEGER) {
        Integer *i = dynamic_cast<Integer *>(num);
        this->set_val(this->real()+ (double)i->val(), this->imag());
        delete(i);
        return this;
    } else if (num->types() == FLOAT) {
        Float *f = dynamic_cast<Float *>(num);
        this->set_val(this->real()+ f->val() , this->imag());
	 delete(f);
	 return this;
    } else if (num->types() == COMPLEX) {
        Complex *c = dynamic_cast<Complex *>(num);
        this->set_val(this->real()+ c->real(), this->imag()+ c->imag() );
	 delete(c);
	 return this;
    } else
        return nullptr;
...
Number::type Complex::types() const {
     ...
     */
    return this->type::COMPLEX; // 구현한거 아님..!
}
