...
Number *Integer::add(Number * num) {
	...
	f->set_val((double)this->value + f->val() );
	delete(this);
	return f; //수의 범위가 더 큰 걸로  return
    }
    else if (num->types() == COMPLEX)
    {
	Complex *c = dynamic_cast<Complex *>(num);
	c->set_val((double)this->value + c->real(), c->imag() );
	delete(this);
	return c;
    }
    else //어떠한 타입에도 속하지 않을 경우
	return nullptr;
}
...
Number *Integer::mul(Number * num) {
    ...
    {
        Complex *c = dynamic_cast<Complex *>(num);
        c->set_val((double)this->value * c->real(), (double)this->value * c->imag() );
        delete(this);
        return c;
    }
    else //어떠한 타입에도 속하지 않을 경우
...
Number *Integer::div(Number * num) {
    ...
    else //complex일 경우 or 어떠한 타입에도 속하지 않을 경우
        return nullptr;
}
