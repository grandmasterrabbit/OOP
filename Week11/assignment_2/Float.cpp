...
Number *Float::mul(Number *num) {
    ...
    if (num->types() == INTEGER) {
        ...
        Complex *c = dynamic_cast<Complex *>(num);
        c->set_val(this->value * c->real(), this->value * c->imag());
        delete(this);
        return c;
    } else
        return nullptr;
